...
export default function CreateLinkForm(): JSX.Element {
    ...
    function generateLink(event: React.FormEvent<HTMLFormElement>): void {
        ...
        const { randomUUID }: ShortUniqueId = new ShortUniqueId({length: 10});
        if (!updated) {
    ...
    return (
        <form className='flex flex-col md:max-w-sm max-w-96 mx-auto' onSubmit={generateLink}>
            ...
            {updated ? <div className="rounded-md border border-zinc-300 py-2 mt-8 underline"><Link to={`/list/${data.url}`}>https://example.com/list/{data.url}</Link></div> : null}
        </form>
    )
}
